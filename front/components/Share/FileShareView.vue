...
const handleDownload = async () => {
    ...
    try {
        ...
        if (!token) {
            throw new Error(t('page.shareView.file.tokenError'))
        }
        downloadFile(token)
    } catch (error: any) {
...
const fileShareInfo = computed(() => {
    return [
        { label: t('page.shareView.file.passwordRequired'), value: props?.data?.has_password ?? false },
        {
            label: t('page.shareView.file.expireTime'),
            value: dayjs.duration(remaining.value, 'seconds').format(`D[${t('common.days')}] HH:mm:ss`),
        },
        { label: t('page.shareView.file.remainingDownloads'), value: props?.data?.download_nums ?? 0 },
    ]
})
</script>
...
<template>
    <div class="flex flex-col gap-5 items-center">
        <h1 class="text-xl font-bold">{{ t('page.shareView.file.title') }}</h1>
        ...
        <div class="w-full">
            <AsyncButton @click="handleDownload" class="w-full">{{ t('page.shareView.file.download') }}</AsyncButton>
        </div>
    </div>
</template>
